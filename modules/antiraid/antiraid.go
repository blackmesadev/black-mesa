...
func AntiRaidProcess(s *discordgo.Session, anti structs.AntiRaidThreshold, userID string, guildID string) bool {
	if r == nil {
		r = bmRedis.GetRedis()
	}

	key := fmt.Sprintf("antiRaid:users:%v:%v", guildID, userID)
