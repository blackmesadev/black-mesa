...
use crate::mongo::mongo::{PunishmentType, Config};
use crate::automod::{censor::*, spam::*, clean};
...
impl Handler {
    pub async fn automod(&self, conf: &Config, msg: &AutomodMessage) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    pub async fn run_automod(&self, conf: &Config, msg: &AutomodMessage) -> Option<AutomodResult> {
        ...
        let closest_censor_lvl: i64 = permissions::get_closest_level(censor_levels.keys().cloned().collect(),
        ...
        let closest_spam_lvl: i64 = permissions::get_closest_level(spam_levels.keys().cloned().collect(),
        permissions::get_user_level(conf, Some(&roles), &user_id));
