...
pub fn filter_invites(censor_user: &automod::Censor, content: &String) -> (String, bool) {
    ...
    for word in content_split {
        if word.contains("discord.gg") {
            ...
            if !invites_whitelist.contains(&invite_code.to_string())
                || !invites_whitelist.contains(&format!("discord.gg/{}", invite_code))
            {
...
pub fn filter_blacklisted_invites(
    invites_blacklist: &Vec<String>,
    content: &String,
) -> (String, bool) {
    let content_split = content.split_whitespace();
    for word in content_split {
        if word.contains("discord.gg") {
            let invite_code = word.split("discord.gg/").collect::<Vec<&str>>()[1];
            if invites_blacklist.contains(&invite_code.to_string())
                || invites_blacklist.contains(&format!("discord.gg/{}", invite_code))
            {
                return (invite_code.to_string(), false);
            }
        }
