...
impl Handler {
    pub async fn remove_punishment_cmd(&self, conf: &Config, msg: &Message)
    ...
    pub async fn expire_punishment_cmd(&self, conf: &Config, msg: &Message)
    -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let content = &msg.content;
        lazy_static! {
            static ref RE: Regex = Regex::new(r"\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}\b").unwrap();
        ...
        for punishment in &punishment_list {
            if &punishment.issuer == &author_id {
                let ok = permissions::check_permission(conf, roles, &author_id, vec![permissions::PERMISSION_EXPIREACTIONSELF]);
                    if !ok {
                        self.rest.create_message(msg.channel_id)
                ...
                let ok = permissions::check_permission(conf, roles, &author_id, vec![permissions::PERMISSION_EXPIREACTION]);
                if !ok {
                    self.rest.create_message(msg.channel_id)
        ...
        match res {
            UpdateResult{modified_count, ..} => {
                if modified_count < 1 {
                    self.rest.create_message(msg.channel_id)
                        .content(format!("<:mesaCross:832350526414127195> Unable to expire `{}` punishments", punishment_list.len())
                        .as_str())?
                        
                        .await?;
                } else {
                    self.rest.create_message(msg.channel_id)
                        .content(format!("<:mesaUnstrike:869664457788358716> Expired `{}` punishments", modified_count).as_str())?
